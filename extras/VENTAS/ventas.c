...
void generarCompra(int idUsuario, float gasto)
{
    ...
    } while (verificarSaldo(saldo, gasto) == 0);

    if (verficarSaldo(saldo, gasto) == 1)
    {
        /// descuenta del saldo.
    ...
    if (bufferUsuarios && bufferVentas)
    {
        while (fread(&aux, sizeof(usuario), 1, buffer) > 0)
        {
            if (deseado.idCliente == aux.idCliente)
            {
                ...
                pedirDatosEnvio(aux.[aux.validosCompras].compras.despachar);
...
void cambiarEstadoSubVenta(char nombreProducto[], int idUsuario)
{
    ...
    FILE *bufferUsuario(archivoUsuarios, "r+b");
    usuario Aux;
    if (bufferUsuario)
    {
...
venta buscarVentaEnRegistroUsuario(int idVenta, int idUsuario)
{
    ...
    if (bufferUsuario)
    {
        while (fread(&temporal, sizeof(usuario), 1, buffer) > 0 && flag == 0)
        {
...
int generarIdVenta()
{
    ...
    if (bufferVentas)
    {
        ...
        while (fread(&Aux, sizeof(ventas), 1, buffer) > 0)
        {
            id++;
        }
        fclose(bufferVentas);
...
nodoVentaD *despersistirListaDobleVentasExitosas(nodoVentaD *lista)
{
    ...
    if (ptr != NULL)
    {
        ...
        {
            if (aux.estadoCliente == 1 && aux.estadoVenta == 0)
            {
